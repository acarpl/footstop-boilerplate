...
export class DashboardService {
    ...
    const totalRevenueResult = await this.orderRepo.sum('total_price', { status_pengiriman: 'Delivered' });
    
    // GANTI 'statusPengiriman' menjadi nama properti yang benar di Order entity (misal: 'status_pengiriman')
    const newOrders = await this.orderRepo.count({ where: { status_pengiriman: 'Dibayar' }});
