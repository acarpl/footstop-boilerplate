...
export class PaymentsService {
  ...
  constructor(
    ...
    private readonly orderRepository: Repository<Order> // Inject DataSource jika Anda perlu transaksi untuk handle webhook
  ) {
  ...
  async createMidtransTransaction(user: User, orderId: number) {
    ...
    const parameter: midtransClient.Snap.TransactionParameter = {
      transaction_details: {
        order_id: `FOOTSTOP-${order.id_order}-${Date.now()}`, // Buat ID unik untuk Midtrans
      ...
      item_details: order.orderDetails.map((item) => ({
        ...
        name: item.product.product_name.substring(0, 50), // Nama item maks 50 char
      })),
    };
  ...
  async handleMidtransNotification(notification: any) {
    ...
    console.log(
      `Order #${orderId} status updated to ${order.status_pengiriman}`
    );
    return { status: "ok" };
  }
}
